...
def train_model(X, y):
    ...
    rmse = mse ** 0.5
    print(f"Root Mean Squared Error (RMSE) dari model Random Forest: {rmse}")

    return model, X_test, y_test

def visualize_decision_tree_sklearn(model, feature_names):
    # Ambil salah satu pohon keputusan dari RandomForest
    estimator = model.estimators_[0]

    plt.figure(figsize=(15, 10))  # Perbesar ukuran gambar agar lebih rapi
    plot_tree(estimator, feature_names=feature_names, filled=True, rounded=True, fontsize=10)
    plt.show()
...
def test_model(model, X_full, y_full):
    ...
    plt.title('Accuracy of Model Predictions with Random Forest')
    ...
    # Menampilkan hasil perbandingan antara rating aktual dan prediksi
    for actual, predicted in zip(y_full, predicted_ratings_rounded):
...
def main():
    ...
    visualize_decision_tree_sklearn(model, X_full.columns)
